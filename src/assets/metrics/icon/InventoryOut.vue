<script setup lang="ts">
import Icon from "@ant-design/icons-vue";
</script>

<template>
  <icon>
    <template #component="{fill,height,width,style}">
      <svg viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"
           :width="width" :height="height" :style="style">
        <path
            d="M949.57 423.68L533.7 70.85a32 32 0 0 0-41.4 0L76.43 423.68a32 32 0 0 0 41.4 48.8l11.07-9.39v463.65a32 32 0 0 0 64 0V409.31v-0.5L513 137.22l320.16 271.63v198.24a32 32 0 0 0 64 0V463.14l11 9.34a32 32 0 1 0 41.4-48.8z"
            :fill="fill"/>
        <path d="M256.85 862.49m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path d="M400.86 862.49m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path d="M257.58 735.38m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path d="M257.58 607.9m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path d="M400.86 734.65m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path d="M544.86 862.49m12 0l72 0q12 0 12 12l0 72q0 12-12 12l-72 0q-12 0-12-12l0-72q0-12 12-12Z"
              :fill="fill"/>
        <path
            d="M896.14 782.1V781.47v-0.56V780.26c0-0.21-0.05-0.43-0.08-0.64s0-0.32-0.06-0.47V779c0-0.23-0.08-0.47-0.12-0.7s0-0.26-0.07-0.4v-0.1c0-0.24-0.1-0.47-0.16-0.71s-0.05-0.24-0.08-0.37v-0.12l-0.18-0.68-0.1-0.36v-0.14c-0.06-0.21-0.13-0.42-0.2-0.62l-0.13-0.39-0.06-0.17-0.21-0.55-0.16-0.42-0.08-0.19-0.21-0.47-0.2-0.46-0.1-0.21-0.19-0.39-0.23-0.47-0.12-0.21-0.17-0.3-0.06-0.11-0.25-0.45-0.13-0.21-0.15-0.24-0.11-0.18-0.53-0.38-0.13-0.18-0.14-0.21-0.16-0.23-0.28-0.4-0.11-0.15-0.16-0.2-0.2-0.26-0.3-0.39-0.09-0.11-0.21-0.24-0.21-0.25-0.32-0.37-0.33-0.35-0.29-0.3-0.24-0.25-64-63.69a25 25 0 1 0-35.26 35.45l21.07 21H679.12a25 25 0 0 0 0 50H810.7L789.43 829a25 25 0 0 0 35.31 35.4l64.07-63.92 0.24-0.25 0.55-0.58 0.06-0.07 0.36-0.41 0.38-0.44 0.09-0.11 0.33-0.42 0.18-0.2 0.25-0.33 0.12-0.15 0.31-0.44 0.07-0.1 0.21-0.3 0.13-0.19 0.29-0.46v-0.07l0.2-0.31 0.13-0.21 0.27-0.48 0.2-0.36 0.12-0.22 0.23-0.46 0.21-0.42 0.1-0.21 0.19-0.43 0.21-0.49 0.08-0.19 0.15-0.4 0.21-0.57 0.06-0.17 0.12-0.37c0.07-0.21 0.14-0.42 0.21-0.64v-0.15l0.1-0.35c0.06-0.23 0.13-0.45 0.18-0.68v-0.13c0-0.12 0.05-0.24 0.08-0.36s0.11-0.47 0.15-0.71v-0.11c0-0.13 0-0.26 0.07-0.39s0.08-0.46 0.12-0.69v-0.1c0-0.16 0-0.31 0.06-0.47s0.06-0.42 0.08-0.63v-0.09-0.61V783.97v-0.62-0.55c0.24-0.29 0.23-0.5 0.23-0.7z"
            :fill="fill"/>
      </svg>
    </template>
  </icon>
</template>


