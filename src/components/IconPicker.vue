...
let icons = AllMetrics.find((item) => item.type == 'icon' || item.type == '标签')!.items
  ...
  <a-popover v-model:open="visible" trigger="click" placement="bottom">
    <template #content>
      <div class="icons-query">
        <input type="text" class="input" placeholder="Filter 输入以过滤，如：告警"/>
      </div>
    ...
    <div class="selected-icon" @click="visible = true">
