...
const chartConfigFunction = {
  ...
  changeGaugeColor: (gaugeName: string) => {
    ...
    if (gaugeColorObj) {
      ...
      chartOption.series.axisLine.lineStyle.color = colors
...
<template>
  ...
  <div class="chart-group">
    ...
    >
      ...
      >
        ...
        >
          <div
              v-for="(colorItem, colorIndex) in item.colors.length > 7
                ? 7
                : item.colors.length"
              class="color-item"
              :key="colorIndex"
    ...
    <div class="chart-item">
      ...
      <div class="component-right">
        <a-switch
            ...
            @change="(v:any)=>{chartConfig.setOption({title:{show:v.target.checked}})}"
        ></a-switch>
        <a-radio-group
  ...
  <div class="chart-group">
    ...
    <!--标题开关以及位置-->
    <div class="chart-item">
      <a-color-picker v-model:color="chartConfigControl.pickColor[0]"></a-color-picker>
      ...
      >
        <template #header>
          <div :style="{ display: 'flex', justifyContent: 'space-between' }">
            ...
            <span>
              <DashboardOutlined :style="{ color: chartConfigFunction.getColor(index) ,fontSize:'16px'}"/>
            </span>
          </div>
        </template>
      </a-collapse-panel>
